...
## FIX-020 — Admin made fully real: project status, settings persistence, dashboard depth (2026-06-29)
- **Context:** after FIX-019 the inquiries + dashboard were Firestore-backed, but three admin surfaces were still mock/non-functional: the Projects "Live/In progress" badges (hardcoded index array + dead "Edit" button), the Settings form (no persistence — "Save" was a toast), and the dashboard only had four KPIs.
- **Projects status:** added a `status` field (`"Live" | "In progress" | "Maintenance"`) to the `Project` type and to every entry in `src/data/projects.ts` (all six ship a live URL → "Live"). `projects-view.tsx` now reads `project.status` (mapped to the badge palette) and the dead "Edit" button was removed; `MOCK_PROJECT_STATUS` deleted.
- **Settings persistence:** new `settings/site` Firestore doc via `src/lib/settings.ts` (`getSiteSettings` merges saved over `site.config.ts` defaults; `saveSiteSettings`) and `src/app/admin/settings-actions.ts` (`saveSettings`, re-checks `requireAdmin`, validates hex/length). The page fetches settings and threads them through `AdminViews` → `SettingsView`, which is now controlled, persists profile + appearance, restores saved appearance on mount, and shows real success/error toasts. Note: profile values are an admin record — the public site still renders statically from `site.config.ts`.
- **Dashboard depth:** `dashboard-stats.ts` now also computes a secondary metric strip — new-this-week, leads by source (contact vs intake), and open pipeline value (sum of intake estimates for new/following-up leads) — rendered under the KPI grid.
- **Sample data:** seeded 6 clearly-marked sample leads (`sample: true`, "Sample —" names, `@example.com`) spanning sources/statuses/dates so the populated admin can be demoed; purge via the `sample == true` flag.
- **Verified:** end-to-end on a dev server against live Firebase — inquiries view shows all 6 seeded leads with correct status/date; pipeline + counts aggregate correctly; projects show real "Live" badges; settings save → persist across reload → revert. type-check/lint/build green; npm audit 0. (CountUp/Reveal in-view animations don't fire under the headless preview's IntersectionObserver, but render normally in real browsers.)
